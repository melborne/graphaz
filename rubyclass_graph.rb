...
obj, mod, cla, sta, ins_a, ins_b, ins_c, ker, modA, modB =
  "Object\nclass", "Module\nclass", "Class\nclass", "Standard\nclass",
  "Instance a", "Instance b", "Instance c", "Kernel\nmodule",
  "module A", "module B"
...
ga.lap
ga.edge( "#{cla}_#{mod}", "#{cla}_#{obj}", "#{cla}_#{sta}",
         :style => 'bold', :color => 'steelblue' )
ga.lap
ga.node( obj, mod, sta,
         :style => 'filled', :fillcolor => 'steelblue' )
ga.lap
ga.edge( "#{sta}_#{ins_a}", "#{sta}_#{ins_b}", "#{sta}_#{ins_c}",
        "#{mod}_#{modA}", "#{mod}_#{modB}", "#{mod}_#{ker}",
